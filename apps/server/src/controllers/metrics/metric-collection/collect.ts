...
class MetricCollectionController {
    ...
    private async markMetricCollected(metricId: string, requestData: Omit<MetricCollectionBody, 'orgSlug'>): Promise<boolean> {
        try {
    ...
    collectOrganisationMetric = async (req: express.Request, res: express.Response) => {
        try {
            const {
                ...
                numeric_value,
                conversion_step
            } = req.body as MetricCollectionBody;
            ...
            const metricCollection = await this.markMetricCollected(metricId, {
                metric_key,
                part_rollout,
                variation_served,
                event_type,
